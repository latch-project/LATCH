...
print("Parent being added:", Path.cwd().parents[0])
...
def generate_ai_response(system_content, user_content, provider):
    ...
    try:
        ...
        elif "anthropic" in provider and "5" in provider:

            anthropic_config = {
                "provider": "anthropic",
                "model": provider.split("_")[1],
                "max_tokens": 8096,  # 50000, #4096,
...
def display_variable_metadata_with_year_and_key(schema, year, key, raw_input, examples):
    ...
    if schema =="aireadi":
        years_list = data_config.schema_configs["aireadi"]["available_years"]
        row[0] = str(years_list[0])
        if headers[-1] == "values":
        ...
        years_list = data_config.schema_configs["registry"]["available_years"]
        row[0] = str(years_list[0])
        if headers[-1] == "values":
            print(tabulate([row[:-1]], headers=headers[:-1], tablefmt="grid"))
        else:
...
def _parse_string(value: str) -> Any:
    """
    Convert a dictionary/list stored as text into a Python object.
...
def _find_schema_column(
    ...
    """Find the schema column in a DataFrame or row."""
    if schema_column is not None:
        if schema_column not in df:
            raise KeyError(
                f"Column {schema_column!r} was not found. "
                f"Available columns: {list(df.index if isinstance(df, pd.Series) else df.columns)}"
            )
        return schema_column

    available = df.index if isinstance(df, pd.Series) else df.columns

    for column in DEFAULT_SCHEMA_COLUMNS:
...
def _prepare_values(
    ...
    """
    Normalize a DataFrame, Series, dictionary, list, or CSV cell
    into a list of parsed Python values.
    """
    # Full DataFrame
    if isinstance(value, pd.DataFrame):
        column = _find_schema_column(value, schema_column)

        if column is None:
            raise KeyError(
                "Could not find a schema column. "
                f"Expected one of {DEFAULT_SCHEMA_COLUMNS}. "
                f"Available columns: {value.columns.tolist()}"
            )

        return [
            _parse_string(cell) if isinstance(cell, str) else cell
    ...
    if isinstance(value, pd.Series):
        ...
        # The Series may itself contain schema values
        return [
            _parse_string(cell) if isinstance(cell, str) else cell
            for cell in value.tolist()
            if not _is_missing(cell)
        ]

    # One CSV cell containing a dictionary
    if isinstance(value, str):
        return [_parse_string(value)]

    # Dictionary, list, tuple, or other value
...
def _walk(value: Any):
    """Recursively yield every nested value."""
    yield value
...
def get_cycles(
    ...
    """
    Get all unique cycle values.

    Accepts a DataFrame, Series, dictionary, list, or CSV string.
    """
    cycles = []

    for prepared_value in _prepare_values(value, schema_column):
        for obj in _walk(prepared_value):
            ...
            # Fallback for malformed, unparsed strings
            elif isinstance(obj, str):
                matches = re.findall(
                    r"""["']cycle["']\s*:\s*["']([^"']+)["']""",
                    obj,
                )
                cycles.extend(matches)
...
def get_datasets(
    ...
    """
    Get all unique dataset values.

    This is useful when a DataFrame may contain multiple datasets.
    """
    datasets = []
...
def get_dataset(
    ...
    """
    Return the first dataset found, or None.

    Use get_datasets() when multiple datasets may be present.
    """
    datasets = get_datasets(value, schema_column)
    return datasets[0] if datasets else None



def retrieve_candidates(top_n, record):
    schema = get_dataset(record).lower()

    years_list = [
        [int(start), int(end)]
        for start, end in (
            cycle.split("-") for cycle in get_cycles(record)
        )
    ]

    not_found_keywords = get_keywords(record)

    config_variable = data_config.config_map[schema]
    model = SentenceTransformer("all-MiniLM-L6-v2")#, device="cpu")

    if schema == "aireadi":
        dictionary_path = data_config.schema_configs["aireadi"]["dictionary"]

    elif schema == "registry":
        dictionary_path = data_config.schema_configs["registry"]["dictionary"]
 
    elif schema == "nhanes":
        dictionary_path = data_config.schema_configs["nhanes"]["dictionary"]

    else:
        raise ValueError(f"Unsupported schema: {schema}")
 
    # Read the dictionary only once.
    full_df = pd.read_csv(dictionary_path)

    all_results = []

    print("Organizing and recording retrieved candidates for each keyword and cycle...")
    ...
    for i, keyword in enumerate(not_found_keywords, start=1):
        ...
        for years in years_list:
            try:
                ...
                keyword_column = config_variable["keyword_column"]

                if df.empty or keyword_column not in df.columns:
                    print(
                        f"No usable data for {schema}, years {years}. "
                        f"Expected column: {keyword_column}"
                    )
                    continue
                ...
                columns_to_keep = config_variable["variable_lookup"]
                ...
                candidates = candidates[
                    columns_to_keep + ["Similarity Score"]
                ]
                ...
                pattern = "|".join(
                    re.escape(word) for word in words_to_drop
                )
                ...
                if schema == "nhanes":
                    column_mapping = {
                        "SAS Label": "Available Variable",
                        "Data File Description": "Table Description",
                        "Data File Name": "Table Name",
                        "Examples": "Examples",
                    }
                else:
                    column_mapping = {
                        "column_name": "Available Variable",
                ...
                organized_candidates = (
                    candidates[
                        available_columns + ["Similarity Score"]
                    ]
                    .head(top_n)
                    .copy()
                ...
                # Add identifying information to every result.
                organized_candidates.insert(
                    0,
                    "Search Keyword",
                    keyword,
                )
                ...
                all_results.append(organized_candidates)

            except KeyError as error:
                print(
                    f"[Error] Missing column for keyword "
                    f"'{keyword}', cycle {years}: {error}"
                )
                continue

            except Exception as error:
                print(
                    f"[Error] Keyword '{keyword}', "
                    f"cycle {years}: {error}"
                )
                continue
...
def _normalize_table_name(table_name):
    """
    Add the nhanes. prefix when it is missing.

    demo_d         -> nhanes.demo_d
    nhanes.demo_d  -> nhanes.demo_d
    """
    ...
    if not table_name.startswith("nhanes."):
        table_name = f"nhanes.{table_name}"
...
def _normalize_cycle(cycle):
    """
    Normalize cycles into strings such as '2005-2006'.
    """
    if isinstance(cycle, (list, tuple)):
        return "-".join(str(year) for year in cycle)

    return str(cycle).strip()


def extract_expected_sources(record):
    """
    Recursively extract every expected:
      keyword + cycle + table + column

    from the input record.
    """
    expected_sources = []

    def walk(obj, parent_keyword=None):
        if isinstance(obj, dict):
            current_keyword = obj.get("keyword", parent_keyword)

            for source in obj.get("data_sources", []):
                expected_sources.append(
                    {
                        "Search Keyword": current_keyword,
                        "Expected Cycle": source.get("cycle"),
                        "Expected Table": source.get("table"),
                        "Expected Column": source.get("column"),
                    }
                )

            for key, value in obj.items():
                if key != "data_sources":
                    walk(value, current_keyword)

        elif isinstance(obj, list):
            for item in obj:
                walk(item, parent_keyword)

    walk(record)

    return (
        pd.DataFrame(expected_sources)
        .drop_duplicates(
            subset=[
                "Search Keyword",
                "Expected Cycle",
...
def find_expected_candidate_ranks(record, candidates_df):
    """
    Find the 1-based candidate rank of every expected variable.

    A match requires:
      1. Search Keyword matches
      2. Cycle matches
      3. Table matches after adding 'nhanes.'
      4. Available Variable matches the expected column after normalization
    """
    required_columns = {
        "Search Keyword",
        "Cycle",
    ...
    if missing_columns:
        raise ValueError(
            f"candidates_df is missing columns: {sorted(missing_columns)}"
        )

    candidates = candidates_df.copy()

    candidates["_Original DataFrame Index"] = candidates.index

    candidates["_Normalized Keyword"] = (
        candidates["Search Keyword"].apply(_normalize_string)
    )

    candidates["_Normalized Cycle"] = (
        candidates["Cycle"].apply(_normalize_cycle)
    )

    candidates["_Normalized Variable"] = (
        candidates["Available Variable"].apply(_normalize_string)
    )

    candidates["_Normalized Table"] = (
        candidates["Table Name"].apply(_normalize_table_name)
    )

    candidates["Candidate Rank"] = (
        candidates
        .groupby(
            ["_Normalized Keyword", "_Normalized Cycle"],
            sort=False,
        )
        .cumcount()
        .add(1)
    ...
    expected_sources = extract_expected_sources(record)

    report = []

    for _, expected in expected_sources.iterrows():
        expected_keyword = expected["Search Keyword"]
        expected_cycle = _normalize_cycle(expected["Expected Cycle"])
        expected_table = _normalize_table_name(expected["Expected Table"])
        expected_column = _normalize_string(expected["Expected Column"])

        normalized_keyword = _normalize_string(expected_keyword)

        keyword_cycle_candidates = candidates[
            (
                candidates["_Normalized Keyword"]
                == normalized_keyword
            )
        ...
        exact_matches = keyword_cycle_candidates[
            (
                keyword_cycle_candidates["_Normalized Table"]
                == expected_table
            )
            & (
                keyword_cycle_candidates["_Normalized Variable"]
                == expected_column
            )
        ]
        ...
        if exact_matches.empty:
            report.append(
                {
                    "Search Keyword": expected_keyword,
                    "Cycle": expected_cycle,
                    "Expected Table": expected_table,
                    "Expected Column": expected["Expected Column"],
                    "Found": False,
                    "Candidate Rank": pd.NA,
                    "DataFrame Index": pd.NA,
                    "Matched Variable": pd.NA,
                    "Matched Table": pd.NA,
                    "Similarity Score": pd.NA,
                }
            )

        else:

            first_match = exact_matches.sort_values(
                "Candidate Rank"
            ).iloc[0]
            ...
            report.append(
                {
                    ...
                    "Found": True,
                    "Candidate Rank": int(
                        first_match["Candidate Rank"]
                    ),
                    "DataFrame Index": int(
                        first_match["_Original DataFrame Index"]
                    ),
                    "Matched Variable": first_match[
                        "Available Variable"
                    ],
                    "Matched Table": _normalize_table_name(
                        first_match["Table Name"]
                    ),
                    "Similarity Score": first_match.get(
                        "Similarity Score",
                        pd.NA,
                    ),
                }
...
def look_up_variables(schema, not_found_keywords, top_n):
    config_variable = data_config.config_map[schema]
    model = SentenceTransformer("all-MiniLM-L6-v2")

    if schema == "aireadi":
        dictionary_path = data_config.schema_configs["aireadi"]["dictionary"]
        years_list = data_config.schema_configs["aireadi"]
        ...
        dictionary_path = data_config.schema_configs["nhanes"]["dictionary"]
        # years_list = data_config.schema_configs["nhanes"]["available_years"]
        years_list = [
            [2021, 2023],   
            [2017, 2020],
            [2019, 2020],
    ...
    for i, keyword in enumerate(not_found_keywords, start=1):
        ...
        for years in years_list:
            try:
                ...
                keyword_search = df[keyword_column].fillna("").astype(str).tolist()
                ...
                candidates = df.iloc[indices[0]].copy()
                candidates["Similarity Score"] = distances[0]
                ...
                organized_candidates = (
                    candidates[list(column_mapping)]
                    .head(top_n)
                    .copy()
                    .rename(columns=column_mapping)
                )
                ...
                print(f"\nCycle Year: {years}")
                print(organized_candidates.to_string(index=False))

            except KeyError as error:
                print(f"[Error] Missing column: {error}")
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if "aireadi" in schema:
                        years = data_config.schema_configs["aireadi"]["available_years"][0]
                        cycle = f"{years[0]}-{years[1]}"

                    elif "registry" in schema:
                        years = data_config.schema_configs["registry"]["available_years"][0]
                        cycle = f"{years[0]}-{years[1]}"
...
def process_sql_oneshot(
    ...
    return None, combined_sql, None, 1, usage_logs, None
