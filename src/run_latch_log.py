    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            try:
                ...
                if (
                    variable_matcher is not None
                    and getattr(variable_matcher, "summary", None) is not None
                ):
                    try:
                        run_values["Candidates"] = (
                            utils.retrieve_candidates(
                                top_n=40,
                                record=variable_matcher.summary,
                            ).to_dict(orient="records")
                        )
                    except Exception as candidate_error:
                        print(f"[ERROR] Candidate generation failed: {candidate_error}")
            ...
            # Generate candidates after total time
            if (
                variable_matcher is not None
                and getattr(variable_matcher, "summary", None) is not None
            ):
