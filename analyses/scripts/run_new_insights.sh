...
RESULT_FOLDER="$ROOT_DIR/analyses/results/fig4_new_insights"
PROMPT_DIR="$ROOT_DIR/analyses/prompts/fig4_new_insights"
RUNNER_PY="$ROOT_DIR/src/run_latch.py"
LLM_PROVIDER="google_gemini-2.5-flash"

for PROMPT_FILE in "$PROMPT_DIR"/4*.txt; do
  BASENAME="$(basename "$PROMPT_FILE" .txt)"
  ANALYSIS_NAME="fig_4_${BASENAME}"
